...
PRO readraw,configvars,raw,scan_index,sweep_count,transition_flag,elevation,azimuth,n_blocks,n_pol,scatvars,$
...
PRO compute_range_profiles,range_gate_spacing,n_blocks,n_pol,ngates,gate_offset,gate_plot_max,elevation,azimuth,scan_index,$
     ...
     ENDFOR 
      ...
      IF configvars.i_raw_plot EQ 1 and scan_index(i) eq 1 THEN BEGIN 
         IF sweep_count(i) NE sweep_count_old THEN BEGIN 
            plot_range_profiles,i,spec,sweep_count,ngates,gate_offset,gate_plot_max,geometric_peak,elevation,azimuth,range
         ENDIF 
      ENDIF
      if configvars.i_raw_plot eq 2 and abs(scan_index(i)) eq 1 or configvars.i_raw_plot eq 3 then plot_range_profiles,i,spec,sweep_count,ngates,gate_offset,gate_plot_max,geometric_peak,elevation,azimuth,range
...
PRO plot_range_profiles,i,spec,sweep_count,ngates,gate_offset,gate_plot_max,geometric_peak,elevation,azimuth,range
;
...
;print,'***********************************'
  ...
  oplot,range(gate_offset:gate_plot_max),pspec_hh(gate_offset:gate_plot_max),color=80
  oplot,range(gate_offset:gate_plot_max),pspec_vh(gate_offset:gate_plot_max),color=40
  if elevation(i) le 89.50 then begin
   ...
   xyouts,.85*range(gate_plot_max),yoff2-10,'azimuth angle (deg): ',alignment=1,charsize=1.4
   xyouts,.85*range(gate_plot_max),yoff2-10,strtrim(string(round(azimuth(i))),2),charsize=1.4
   endif
;cal signal
waitkey,dum
goto,skipcalplot
...
PRO equalize,scatvars,padfac,H_k_norm
  ...
  if scatvars.decimation_mode eq 0 then begin 
     ...
     h_k_norm=congrid(H_k_norm1,N_fft,/interp)
  endif else begin
     h_k_norm=1.0
  endelse
...
plot,range(gate0:gate1),10*alog10(avespec_vv(gate0:gate1)),xtitle='range (m)',ytitle='relative power (dB)',title='ave. power VV blk; HH red; VH blue; HV grn',charsize=1.4,xs=1,ys=1,yr=[yrmin,yrmax];,xr=[1.0,3.0]
